...
```
TemporalDifferenceActorCritic.new(EligibilityTrace: number, discountFactor: number): ModelObject
```

#### Parameters:

* EligibilityTrace: At 0, the model acts like the Temporal Difference algorithm. At 1, the model acts as Monte Carlo algorithm. Between 0 and 1, the model acts as both. [Default: 0]
...
```
TemporalDifferenceActorCritic:setParameters(discountFactor: number)
```
