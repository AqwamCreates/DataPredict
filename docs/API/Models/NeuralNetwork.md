...
Set the number of layers and the neurons (without bias neuron) in each of those layers. Number of arguments determines the layer, while the number value determines the number of neurons. Bias neurons will be added automatically after setting the number of neurons in each layer except the output.
