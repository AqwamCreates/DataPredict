...
Some people want to argue that they don't have to pay for a comprehensive machine learning library and they'll ask the AIs to fully code a DataPredict™ competitor.
...
* For things that are well-known like Neural Networks, they suffer certain code rigidity which would be extremely bad if you want to do optimizations on the existing codes. This is especially noticeable if you have hand-crafted the neural network codes that have all the hand-crafted optimizations. If you deviate from AIs' internal knowledge, the AIs somehow turns a formerly correct and optimized codes for the first derivatives to being wrong or reverted back to whatever AIs' idea of the "correct" implementation for these codes. Even if you tell the AIs' not to touch those codes at all costs and at all times, the AIs will literally change those correct and optimized codes if they see that the codes that the AIs generated themselves produces wrong results. They will literally insist that the reason for incorrect results was due to the previous codes prior to AIs' code changes and will literally modify the formerly correct and optimized codes into wrong or unoptimized ones.
