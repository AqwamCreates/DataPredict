...
| Attempts of reverse engineering or replicating the library, regardless of the presence of other libraries of similar functionalities. | ∞                 | It is extremely unlikely for an employee find it is worth it to replicate for whole library when they can use the library directly. The only way for this action to make sense is to cover the tracks once the library is replicated or the employee leaves middle of the replication. |
