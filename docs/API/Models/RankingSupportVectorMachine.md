# [API Reference](../../API.md) - [Models](../Models.md) - RankingSupportVectorMachine
...
```
RankingSupportVectorMachine.new(maximumNumberOfIterations: integer, cValue: number, kernelFunction: string, kernelParameters: table): ModelObject
```
...
```
RankingSupportVectorMachine:setCValue(cValue: number)
```
...
```
RankingSupportVectorMachine:setSolver(Solver: SolverObject)
```
