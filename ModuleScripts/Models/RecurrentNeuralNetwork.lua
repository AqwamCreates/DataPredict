...
function LongShortTermMemoryModel:backwardPropagateCell(daNext, dcNext, aNext, cNext, aPrevious, cPrevious, ft, it, cct, ot, xt)
	...
	local WfTranposed = AqwamMatrixLibrary:transpose(self.Wf)
	...
	local daPreviousPart1 = AqwamMatrixLibrary:dotProduct(WfTranposed, dft)
	
	local daPreviousPart2 = AqwamMatrixLibrary:dotProduct(WiTransposed, dit)
	
	local daPreviousPart3 = AqwamMatrixLibrary:dotProduct(WcTransposed, dcct)
	
	local daPreviousPart4 = AqwamMatrixLibrary:dotProduct(WoTransposed, dot)
	...
	local dxtPart1 = AqwamMatrixLibrary:dotProduct(WfTranposed, dft)
	
	local dxtPart2 = AqwamMatrixLibrary:dotProduct(WiTransposed, dit)
	
	local dxtPart3 = AqwamMatrixLibrary:dotProduct(WcTransposed, dcct)
	
	local dxtPart4 = AqwamMatrixLibrary:dotProduct(WoTransposed, dot)
...
function LongShortTermMemoryModel:train(tokenInputSequenceArray, tokenOutputSequenceArray)
	...
	if (self.ModelParameters) then
		...
		self.bf = AqwamMatrixLibrary:createMatrix(self.hiddenSize, 1, 1)

		self.Wi = self:initializeMatrixBasedOnMode(self.inputSize, self.hiddenSize + self.inputSize)

		self.bi = AqwamMatrixLibrary:createMatrix(self.inputSize, 1, 1)

		self.Wc = self:initializeMatrixBasedOnMode(self.hiddenSize, self.hiddenSize + self.inputSize)

		self.bc = AqwamMatrixLibrary:createMatrix(self.hiddenSize, 1, 1)

		self.Wo = self:initializeMatrixBasedOnMode(self.hiddenSize, self.hiddenSize + self.inputSize)

		self.bo = AqwamMatrixLibrary:createMatrix(self.hiddenSize, 1, 1)
	...
	repeat
		...
		for t = tokenInputSequenceLength, 1, -1 do
			...
			print(AqwamMatrixLibrary:getSize(daNext))
			
			print(AqwamMatrixLibrary:getSize(daPrevious))
			...
			dby = AqwamMatrixLibrary:add(self.learningRate, dby)
			...
			cost = cost + AqwamMatrixLibrary:sum(dWy)
